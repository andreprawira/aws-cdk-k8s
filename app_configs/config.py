...
from app_configs.models import InfrastructureSpec
...
class AppConfigs:
    ...
    def get_infrastructure_info(self, account_name: str) -> InfrastructureSpec:
        ...
        data_raw = self.from_yaml("app_configs/infrastructure.yaml", context=context)
        data: dict = to_dict(data_raw)
        globals = data.get("globals", {})
        accounts = data.get("accounts", {})
        account = next((x for x in accounts if x["account_name"] == account_name), {})

        # Merge two dictionaries
        merged_config = update(globals, account)
        return from_dict(data_class=InfrastructureSpec, data=merged_config)
